...
class Rpm < Formula
  ...
  def install
    ...
    system "./configure", "--disable-dependency-tracking", "--prefix=#{prefix}", "--with-external-db", "--sysconfdir=#{HOMEBREW_PREFIX}/etc", "--without-hackingdocs", "--enable-python", "--localstatedir=#{HOMEBREW_PREFIX}/var"
    system "make"
    system "make install"
